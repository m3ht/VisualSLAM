function varargout = runkitti(data, pauseLength, makeVideo)
...
% sequence base directory
  base_dir = '/home/fwirth/Desktop/eecs568/eecs-568-project/kitti/data/2011_09_26/2011_09_26_drive_0035_sync';

% load oxts data
Data =  loadOxtsliteData(base_dir);

% transform to poses
pose = convertOxtsToPose(Data)
...
% % 		fast1_update_sim(z);
%     end
    State.Ekf.mu = State.Ekf.mu + u * Param.deltaT
    plotMarker([State.Ekf.mu(1) State.Ekf.mu(2)],'green');
% 
% 	if strcmp(Param.slamAlgorithm, 'ekf')
...
function u = getControl(t,base_dir)
	global Data;
    temp = Data(t)
	u = [-temp{1}(8) -temp{1}(7) 0 ]';
end
